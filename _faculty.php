...
echo "<table>";
echo "<tr><th>Course Name</th><th>Course Code</th><th>Exam</th><th>Question Paper</th><th>Status</th><th>Feedback</th><th>Action</th></tr>";
while ($row = mysqli_fetch_assoc($result)) {
    echo "<tr>";
    echo "<td>" . $row['coursename'] . "</td>";
    echo "<td>" . $row['coursecode'] . "</td>";
    if($row['exam'] == 'InSem') {echo "<td>" . "InSem" . "</td>";}
    if($row['exam'] == 'EndSem') {echo "<td>" . "EndSem" . "</td>";}
...
if (isset($_POST['newupload'])) {
    ...
    do{
        ...
        $sql = "UPDATE if_uploaded_files SET file_name = '$file_name', file_path = '$file_path', status1='-', feedback='-' WHERE id='$id'";
        if (mysqli_query($conn, $sql)) {
